<?php 
$title='User Management'; $this->load->view('_partials/dashboard_header', compact('title')); ?>
        ...
        <div class="d-flex justify-content-md-end flex-wrap gap-2">
          <form method="get" class="d-inline-block me-2">
            <div class="s7__nav-search-form">
              <input type="text" name="q" value="<?= htmlspecialchars($q ?? '') ?>" placeholder="Search username / email / name...">
              <button type="submit" title="Search"><i class="fas fa-search"></i></button>
            </div>
          </form>
          <?php if (!empty($q)): ?>
            <a class="btn btn-link text-muted me-2" href="<?= site_url('users') ?>">Reset</a>
          <?php endif; ?>
      ...
      <table class="table s7__table table-hover align-middle mb-0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <?php if (($u->role ?? '') === 'admin'): ?>
                <span class="badge-soft badge-soft-primary">admin</span>
              <?php else: ?>
                <span class="badge-soft badge-soft-info"><?= htmlspecialchars($u->role ?? 'editor') ?></span>
              <?php endif; ?>
            </td>
            <td>
              ...
              <?php else: ?>
                <span class="badge-soft badge-soft-secondary"><?= htmlspecialchars($u->status ?? 'inactive') ?></span>
              <?php endif; ?>
            </td>
            <td class="mono"><?= htmlspecialchars($u->last_login ?? '-') ?></td>
            <td>
              <a href="<?= site_url('users/edit/'.((int)($u->id ?? 0))) ?>" class="btn btn-outline-light btn-sm s7__btn-icon me-1" title="Edit">
                <i class="fas fa-edit"></i>
              </a>
              <a href="<?= site_url('users/delete/'.((int)($u->id ?? 0))) ?>" class="btn btn-outline-light btn-sm s7__btn-icon"
